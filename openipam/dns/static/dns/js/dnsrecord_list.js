$(function () {
  ...
  var formActionCheck = function () {
    ...
    if (
      ...
      $("tr.add-row:not(.extra)").length > 0 ||
      anyEditable.length > 0
    ) {
      $("#form-actions").show();
    } else {
      $("#form-actions").hide();
  ...
  $("#action-submit").on("click", function () {
    ...
    if (records.length > 0) {
      if (action == "delete") {
        var res = confirm(
          "Are you sure you want to delete the selected dns records?"
        );
        if (res == true) {
          return true;
        }
