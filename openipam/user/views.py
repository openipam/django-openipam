...
class UserManagerJson(PermissionRequiredMixin, BaseDatatableView):
        ...
        try:
            ...
            if ipam_admin_search:
                if ipam_admin_search == '1':
                    qs = qs.filter(Q(is_superuser=True) | Q(groups__name=CONFIG.get('ADMIN_GROUP'))).distinct()
                else:
                    qs = qs.filter(is_superuser=False).exclude(groups__name=CONFIG.get('ADMIN_GROUP'))
            if source_search:
    ...
    def prepare_results(self, qs):
        ...
        for user in qs:
            json_data.append([
                '<input class="action-select" name="selected_users" type="checkbox" value="%s" />' % user.pk if self.request.user.is_superuser else '',
                ...
                '' if user.last_login and user.last_login.year == 1970 else timezone.localtime(user.last_login).strftime('%Y-%m-%d %I:%M %p'),
            ])
        return json_data
