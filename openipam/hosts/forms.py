...
class HostForm(forms.ModelForm):
    ...
    expire_days = forms.ModelChoiceField(
        label="Expires", queryset=ExpirationType.objects.all()
    )
    address_type = forms.ModelChoiceField(queryset=AddressType.objects.all())
    network_or_ip = forms.ChoiceField(
        required=False,
        choices=NET_IP_CHOICES,
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        self.address_types_with_ranges_or_default = [
            address_type.pk
            for address_type in AddressType.objects.filter(
                Q(ranges__isnull=False) | Q(is_default=True)
            ).distinct()
        ]
    ...
    def _init_owners_groups(self):
        if self.instance.pk:
            ...
            if "user_owners" in self.previous_form_data:
                self.fields["user_owners"].initial = self.previous_form_data.get(
                    "user_owners"
                )
            if "group_owners" in self.previous_form_data:
                self.fields["group_owners"].initial = self.previous_form_data.get(
                    "group_owners"
                )
        else:
    ...
    def _init_address_type(self):
        ...
        if not self.user.is_ipamadmin and self.fields.get("address_type"):
            ...
            if self.user_nets:
                r_list = [
                    Q(range__net_contains_or_equals=net.network)
                    for net in self.user_nets
                ]
                range_nets = NetworkRange.objects.filter(reduce(operator.or_, r_list))

                n_list = [
                    Q(network__net_contains_or_equals=nr.range) for nr in range_nets
                ]
                if n_list:
                    other_networks = (
                        True
                        if self.user_nets.exclude(reduce(operator.or_, n_list))
                        else False
                    )
                else:
            ...
            user_address_types = AddressType.objects.filter(
                ...
                | Q(ranges__in=range_nets)
                | Q(pk=existing_address_type)
                | Q(is_default=True if other_networks else None)
            ).distinct()
        ...
        if self.previous_form_data and "address_type" in self.previous_form_data:
            self.fields["address_type"].initial = self.previous_form_data.get(
                "address_type"
            )
    ...
    def _init_network(self):
        ...
        if self.data.get("address_type"):
            self.fields["network"].queryset = Network.objects.by_address_type(
                AddressType.objects.get(pk=self.data["address_type"])
            ).filter(network__in=self.user_nets)

        # Set networks based on address type if form is not bound
        elif self.instance.pk and not self.data:
            # Set address_type
            self.fields["network"].queryset = Network.objects.by_address_type(
                self.instance.address_type
            ).filter(network__in=self.user_nets)

        else:
            self.fields["network"].queryset = Network.objects.filter(
                network__in=self.user_nets
            )
    ...
    def _init_attribute_category(self, category: str, attribute_fields):
        ...
        if self.instance.pk:
            attribute_initials += self.instance.structured_attributes.values_list(
                "structured_attribute_value__attribute", "structured_attribute_value"
            )
        ...
        for attribute_field in attribute_fields:
            ...
            if attribute_field.structured:
                ...
                if attribute_field.multiple:
                    ...
                    self.fields[attribute_field_key] = forms.ModelChoiceField(
                        queryset=attribute_choices_qs, required=False
                    )
            else:
            ...
            if initial:
                ...
                self.previous_form_data
                and attribute_field_key in self.previous_form_data
            ):
                self.fields[attribute_field_key].initial = self.previous_form_data.get(
                    attribute_field_key
                )
    ...
    def _init_expire_date(self):
        if self.instance.pk:
            ...
            """
                % localtime(self.instance.expires.replace(tzinfo=utc)).strftime(
                    "%b %d %Y"
                )
            )
            ...
            self.fields["expire_days"].initial = self.previous_form_data.get(
                "expire_days"
            )
...
#        accordion_groups.append(AccordionGroup(*self.attribute_field_keys))
    ...
    def clean(self):
        ...
        if not cleaned_data["user_owners"] and not cleaned_data["group_owners"]:
            raise ValidationError(
                "No owner assigned. Please assign a user or group to this Host."
            )
    ...
    def clean_network_or_ip(self):
        ...
        if address_type:
            if (
                address_type.pk in self.address_types_with_ranges_or_default
                and not network_or_ip
            ):
    ...
    def clean_network(self):
        ...
        if (
            address_type
            and address_type.pk not in self.address_types_with_ranges_or_default
        ):
            return network
...
class HostRenewForm(forms.Form):
    expire_days = forms.ModelChoiceField(
        label="Expires",
        queryset=ExpirationType.objects.all(),
...
class HostAttributesCreateForm(forms.Form):
    ...
    choice_value = forms.ModelChoiceField(
        label="Value", required=False, queryset=StructuredAttributeValue.objects.all()
    )
