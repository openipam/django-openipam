...
class HostTest(IPAMTestCase):
    def setUp(self):
        ...
        self.dns_domains = [
            {"name": i, "type": "NATIVE"}
            for i in ["valid", "invalid", "168.192.in-addr.arpa"]
        ]

        self.dns_records = [
            {
                "name": "existing-a.invalid",
                "dns_type": "A",
                "ip_content": "192.168.1.2",
            },
            {
                "name": "existing-cname.invalid",
                "dns_type": "CNAME",
                "text_content": "existing-a.invalid",
            },
            {
                "name": "existing-txt.invalid",
                "dns_type": "TXT",
                "text_content": "ignored value",
            },
            {
                "name": "_someproto._tcp.existing-srv.invalid",
                "dns_type": "SRV",
                "priority": 0,
                "text_content": "5 12345 existing-a.invalid",
            },
        ]

        self.hosts = [
            {
                "hostname": "existing-host.invalid",
                "mac": "ffffff000000",
                "address": "192.168.0.3",
            }
        ]
    ...
    def change_and_test(self, hostname=None, mac=None, ip_address=None, instance=None):
        ip_h = Host.objects.add_or_update_host(
    ...
    def test_add_existing_mac(self):
        with self.assertRaises(IntegrityError):
            ...
            Host.objects.create(
                changed_by=self.user_model,
                hostname="not-existing.invalid",
                mac="ffffff000000",
                expires=self.expires,
            )

    def test_create_new_dynamic_host(self):
        pool_h = Host.objects.add_or_update_host(
            user=self.user_model,
            hostname="new-pool.valid",
            expire_days=7,
            mac="123456123456",
            pool="pool1",
        )
    ...
    def test_create_new_static_host(self):
        # Create host
        self.change_and_test(
            hostname="new-ip.valid", mac="001020304050", ip_address="192.168.1.2"
        )

    def test_change_static_host_ip(self):
        # Create host
        h = self.change_and_test(
            hostname="new-ip.valid", mac="001020304050", ip_address="192.168.1.2"
        )

        # Change IP
        h = self.change_and_test(
            hostname="new-ip.valid",
            mac="001020304050",
    ...
    def test_change_static_host_hostname(self):
        ...
        # Change hostname
        h = self.change_and_test(
            "newer-ip.valid", "001020304050", "192.168.1.2", instance=h
        )
    ...
    def test_change_static_host_mac(self):
        ...
        # Change MAC
        h = self.change_and_test(
            "new-ip.valid", "001020304050", "192.168.1.2", instance=h
        )
    ...
    def test_change_static_ip_hostname(self):
        ...
        # Change ip, hostname
        h = self.change_and_test(
            "still-newer-ip.valid", "ab0123456789", "192.168.1.10", instance=h
        )
    ...
    def test_change_static_ip_mac(self):
        ...
        # change ip, mac
        h = self.change_and_test(
            "newer-ip.valid", "ab0123ffffff", "192.168.1.99", instance=h
        )
    ...
    def test_change_static_hostname_mac(self):
        ...
        # change hostname, mac
        h = self.change_and_test(
            "still-newer-ip.valid", "ab0000000000", "192.168.1.7", instance=h
        )
    ...
    def test_add_addresses(self):
        ...
        host.add_ip_address(
            self.user_model,
            hostname="new-ip-additional.valid",
            ip_address="192.168.1.20",
        )
        self.assertEqual(len(host.addresses.all()), 2)
        self.assertEqual(
            str(DnsRecord.objects.get(name="new-ip-additional.valid").ip_content),
            "192.168.1.20",
        )
