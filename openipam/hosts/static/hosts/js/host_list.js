$(function () {
  ...
  $.fn.dataTable.pipeline = function (opts) {
    ...
    return function (request, drawCallback, settings) {
      ...
      if (settings.clearCache) {
        ...
        JSON.stringify(request.order).replace(/(<([^>]+)>)/gi, "") !==
          JSON.stringify(cacheLastRequest.order).replace(/(<([^>]+)>)/gi, "") ||
        JSON.stringify(request.columns).replace(/(<([^>]+)>)/gi, "") !==
          JSON.stringify(cacheLastRequest.columns).replace(
            /(<([^>]+)>)/gi,
            ""
          ) ||
        JSON.stringify(request.search).replace(/(<([^>]+)>)/gi, "") !==
          JSON.stringify(cacheLastRequest.search).replace(/(<([^>]+)>)/gi, "")
      ) {
        ...
        data: function (d) {
          if (!$.isEmptyObject($.urlVars)) {
            $.each(d.columns, function (key, obj) {
              obj.search.value =
                obj.name in $.urlVars ? $.urlVars[obj.name] : "";
            });
          ...
          d.json_data = JSON.stringify(d).replace(/(<([^>]+)>)/gi, "");
        },
        //type: "POST"
      }),
      paging: true,
  ...
  $("#id_search").on("keyup selectChoice", function () {
    // Request autocomplete data from API
    // The endpoint is stored in the data-autocomplete-url attribute
    // of the input field. We need to append the query to it, then
    // make a GET request to the resulting URL.
    var search_value = $(this).val();
    $.get(
      $(this).data("autocomplete-url") + "?q=" + search_value,
      function (data) {
  ...
  $("body").on(
    "click",
    ".autocomplete-light-widget .deck .remove",
    function () {
      ...
      if (toRemove != -1) {
        ...
        $.cookie("search_filter", searchFilter.join(), {
          expires: 1,
          path: "/hosts/",
        });
        results.clearPipeline().draw();
  ...
  $("#host-add-attributes").on(
    "change",
    "select[name='attribute']",
    function () {
      var thisSelect = $(this);
      $.get(
        "/api/web/structuredattributevalues/" + $(this).val(),
        function (data) {
          if (data.trim()) {
            thisSelect
              .parent()
              .parent()
              .parent()
              .find("select[name='attribute_choice']")
              .html(data);
            thisSelect
              .parent()
              .parent()
              .parent()
              .find("input[name='attribute_text']")
              .parent()
              .parent()
              .hide();
            thisSelect
  ...
  $("#network-form-submit").on("click", function () {
    var res = confirm(
      "Are you sure you want to change the network on these hosts?"
    );
    if (res == true) {
      return true;
    }
  ...
  $("#action-submit").on("click", function () {
    ...
    if (hosts.length > 0) {
      if (
        action == "add-owners" ||
        action == "replace-owners" ||
        action == "remove-owners"
      ) {
        $("#host-owners").modal({ backdrop: "static", keyboard: false });
        ...
        var res = confirm(
          "Are you sure you want to populate DNS on the selected hosts?"
        );
        if (res == true) {
          return true;
        }
        ...
        var res = confirm(
          "Are you sure you want export selected hosts to a CSV?"
        );
        if (res == true) {
          return true;
        }
      } else if (action == "renew") {
        $("#host-renew").modal({ backdrop: "static", keyboard: false });
      } else if (action == "rename") {
        $("#host-rename").modal({ backdrop: "static", keyboard: false });
        ...
        $("#host-set-dhcpgroup").modal({ backdrop: "static", keyboard: false });
      } else if (action == "delete-dhcpgroup") {
        var res = confirm(
          "Are you sure you want delete the DCHP group on the selected hosts?"
        );
        if (res == true) {
          return true;
        }
      } else if (action == "change-network") {
        $("#host-change-network").modal({
          backdrop: "static",
          keyboard: false,
        });
      }
