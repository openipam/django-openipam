...
class GuestTicketListCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        instance.user = User.objects.get(username__iexact=validated_data.get('username'))
...
class GuestRegisterSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not mac_address:
            ...
            if not lease:
                raise serializers.ValidationError(mark_safe(
                    "The MAC Address for this guest could not be found. <br />"
                    "Ticket: %s, IP: %s" % (data.get('ticket'), data.get('ip_address'))
                ))
            else:
    ...
    def validate_ticket(self, value):
        ...
        if not valid_ticket:
            raise serializers.ValidationError("Sponsor Code '%s' is expired or invalid" % value)
