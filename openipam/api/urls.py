...
from openipam.api.views.hosts import HostList, HostDetail, HostCreate, HostUpdate, HostOwnerList, HostOwnerAdd, \
    HostOwnerDelete, HostAttributeList, HostAddAttribute, HostDeleteAttribute, HostMac, HostRenew
from openipam.api.views.dns import DomainList
...
urlpatterns = patterns('openipam.api.views',
    ...
    # Hosts
    url(r'^hosts/mac/$', HostMac.as_view(), name='api_host_mac'),
    ...
    url(r'^hosts/(?P<pk>([0-9a-fA-F]{2}[:-]?){5}[0-9a-fA-F]{2})/renew/$', HostRenew.as_view(), name='api_host_renew'),
    ...
    url(r'^domain/$', DomainList.as_view(), name='api_domain_list'),
