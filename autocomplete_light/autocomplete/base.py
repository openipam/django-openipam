...
class AutocompleteInterface(object):
    ...
    def __init__(self, request=None, values=None):
        ...
        if values is None:
            self.values = []
        elif isinstance(values, six.string_types) or not hasattr(values, "__iter__"):
            self.values = [values]
        else:
            self.values = values
    ...
    def get_absolute_url(self):
        ...
        try:
            return reverse(
                "autocomplete_light_autocomplete", args=(self.__class__.__name__,)
            )
...
class AutocompleteBase(AutocompleteInterface):
    ...
    def get_add_another_url(self):
        ...
        if self.add_another_url_name:
            url = reverse(self.add_another_url_name, kwargs=self.add_another_url_kwargs)
            return url + "?_popup=1"
        else:
            return None
