...
class AutocompleteModel(object):
    ...
    def order_choices(self, choices):
        ...
        if self.values:
            pk_name = (
                "id"
                if not getattr(choices.model._meta, "pk", None)
                else choices.model._meta.pk.column
            )

            field = '"%s"."%s"' if connection.vendor == "postgresql" else "%s.%s"
            ...
            return choices.extra(select={ordering_alias: ordering}, order_by=_order_by)
    ...
    def choices_for_values(self):
        ...
        return self.order_choices(
            self.choices.filter(pk__in=[x for x in self.values if x != ""])
        )
    ...
    def choices_for_request(self):
        ...
        assert not isinstance(
            self.search_fields, six.string_types
        ...
        if self.split_words:
            for word in q.strip().split():
                ...
                for search_field in search_fields:
                    word_conditions |= Q(**{self._construct_search(search_field): word})
