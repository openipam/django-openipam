...
class FieldBase(object):
    default_error_messages = {"invalid": "%(autocomplete)s cannot validate %(value)s"}
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.autocomplete = self.get_autocomplete(autocomplete, registry, widget)
        ...
        if (
            forms.ModelChoiceField in parents
            or forms.ModelMultipleChoiceField in parents
        ):
    ...
    def validate(self, value):
        ...
        if value and not self.autocomplete(values=values).validate_values():
            error_params = {"autocomplete": self.autocomplete.__name__, "value": value}

            raise forms.ValidationError(
                self.error_messages["invalid"], code="invalid", params=error_params
            )
...
class ChoiceField(FieldBase, forms.ChoiceField):
    ...
    def __init__(
        ...
        **kwargs
        ...
        kwargs.update({"choices": self.get_choices(autocomplete, registry, widget)})
...
class ModelMultipleChoiceField(ModelChoiceFieldBase, forms.ModelMultipleChoiceField):
    widget = MultipleChoiceWidget
...
class GenericModelMultipleChoiceField(GenericModelChoiceField):
    ...
    def prepare_value(self, value):
        return [
            super(GenericModelMultipleChoiceField, self).prepare_value(v) for v in value
        ]

    def to_python(self, value):
        return [
            super(GenericModelMultipleChoiceField, self).to_python(v) for v in value
        ]
