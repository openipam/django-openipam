...
class AutocompleteRegistry(dict):
    ...
    def register(self, *args, **kwargs):
            ...
            if isinstance(arg, six.string_types):
                ...
                if len(parts) == 2:
                    ...
                    if not issubclass(model, models.Model):
                        raise NonDjangoModelSubclassException(
                            "%s not is subclass of django Model"  # noqa
                            % model.__name__
                        )
                    processed_args.append(model)
                else:
        ...
        if model:
            autocomplete = self._register_model_autocomplete(
                model, autocomplete, derivate_name, **kwargs
            )
        else:
    ...
    def _register_model_autocomplete(
        self, model, autocomplete=None, derivate_name=False, name=None, **kwargs
    ):
        if name is not None:
            pass
        ...
        if base.search_fields is None and "search_fields" not in kwargs:
            try:
                ...
                raise Exception(
                    "Add search_fields kwargs to .register(%s)" % model.__name__
                )
            else:
