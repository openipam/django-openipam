...
class SelectMultipleHelpTextRemovalMixin(forms.BaseModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super(SelectMultipleHelpTextRemovalMixin, self).__init__(*args, **kwargs)
        msg = force_text(M)
    ...
    def _post_clean(self):
        ...
        for field in self._meta.model._meta.private_fields:
            ...
            if value:
                ...
                self.cleaned_data[field.ct_field] = ContentType.objects.get_for_model(
                    value
                )
...
class GenericM2MRelatedObjectDescriptorHandlingMixin(forms.BaseModelForm):
    ...
    def save(self, commit=True):
        ...
        """
        instance = super(GenericM2MRelatedObjectDescriptorHandlingMixin, self).save(
            commit=commit
        )
...
class FormfieldCallback(object):
    ...
    def __call__(self, model_field, **kwargs):
        ...
        if self.autocomplete_exclude and model_field.name in self.autocomplete_exclude:
            pass

        elif (
            self.autocomplete_fields
            and model_field.name not in self.autocomplete_fields
        ):
            pass

        elif hasattr(model_field, "remote_field") and hasattr(
...
class ModelFormMetaclass(DjangoModelFormMetaclass):
    ...
    def clean_meta(cls, meta):
        ...
        for field in fields or []:
            ...
            if (
                ...
                and isinstance(
                    model_field, (RelatedObjectsDescriptor, GenericForeignKey)
                )
            ) or isinstance(model_field, GenericForeignKey):
    ...
    def get_generic_autocomplete(self, meta, name):
        autocomplete_name = getattr(meta, "autocomplete_names", {}).get(name, None)
...
def modelform_factory(
    ...
    **kwargs
    ...
    kwargs["form"] = type(kwargs["form"].__name__, (kwargs["form"],), {"Meta": Meta})
